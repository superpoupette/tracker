...
export function afficherChoreesEnCours(ChoreeEnCours) {
    ...
    for (let i = 0; i < ChoreeEnCours.length; i++) {
        ...
        dureeApprentissage.textContent =
            "Temps d'apprentissage: " + ChoreeEnCours[i].dureeApprentissage + " min";
        ...
        majDureeApprentissage(i, ajouterTemps, ChoreeEnCours);
    }
}


// mise à jour de la durée d'apprentissage des chorées
function majDureeApprentissage(i, ajouterTemps, ChoreeEnCours) {
...
// ajouter une nouvelle chorégraphie dans la base
export function ajouterChoree(mesChorees) {

    let nouvelleChoree = {
        code: ajouterTitre.value + ajouterArtiste.value,
        artiste: ajouterArtiste.value,
        titre: ajouterTitre.value,
        duree: ajouterDuree.value,
